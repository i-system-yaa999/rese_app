...
2. .env.exampleファイルをコピーして.envに改名します。データベース名をrese_appに修正します。

3. mysqlにてデータベースrese_appを作成します。  
コマンドプロンプト： mysql -u root -p  
mysqlコマンド：create database rese_app;

4. マイグレーションをしてテーブルを作成します。  
コマンドプロンプト：cd rese_app  
コマンドプロンプト：php artisan migrate

5. シーディングにてダミーデータを作成します。  
...
作成されるダミーデータは、以下になります。  
    ...
    * エリアデータ：３件
    * ジャンルデータ：１２件
    * ユーザーデータ：１件（メール：user@system.com パスワード：1234567890）
    * ダミーユーザーデータ：１０件(テーブル内データはランダム文字にて作成)
